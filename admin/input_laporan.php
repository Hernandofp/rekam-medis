...
mysqli_select_db($test,"rekam_medis");
		...
		$dtnil=mysqli_query($test, "select * from tb_nilai where id_siswa='$_GET[id]'");
		$bcnil=mysqli_fetch_array($dtnil);
			$dtsw=mysqli_query($test, "select * from tb_siswa where id_siswa='$bcnil[id_siswa]'");
			$bcsw=mysqli_fetch_array($dtsw);
				$dtmpl=mysqli_query($test, "select * from tb_nilai where id_mapel='$bcnil[id_mapel]'");
				$bcmpl=mysqli_fetch_array($dtmpl);
					$dtgr=mysqli_query($test, "select * from tb_nilai where id_guru='$bcnil[id_guru]'");
					$bcgr=mysqli_fetch_array($dtgr);
?>
      ...
      <tr>
        <td valign="top"><table width="100%" border="1" cellpadding="3" cellspacing="0" bordercolor="#000000">
		  ...
		  <?php	
			...
			$no=0;
			$dtn=mysqli_query($test, "select * from tb_nilai where id_siswa='$bcsw[id_siswa]' order by id_mapel asc");
	 		while($bcn=mysqli_fetch_array($dtn)){
			$no++;
				$dtm=mysqli_query($test, "select * from tb_mapel where id_mapel='$bcn[id_mapel]'");
				$bcm=mysqli_fetch_array($dtm);
?>
          <tr>
