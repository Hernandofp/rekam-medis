...
<?php
	...
	$user = $_SESSION['user'];
	$pass = $_SESSION['pass'];
	$sqllogin = mysqli_query($test, "SELECT * FROM tb_user WHERE pengguna='$user' AND sandi='$pass'");
	$cek_log	=	mysqli_fetch_array($sqllogin);
	 		$no=0;
	 			$sql=mysqli_query($test, "SELECT * FROM tb_pendaftaran WHERE stts='1' AND id_dokter='$cek_log[id_dokter]'");
